...
namespace My_Practice_Work.Controllers
{
    ...
    public class My_Controllers_Workers: ControllerBase
    {
        ...
        {
            Worker? user = await db.Workers.FirstOrDefaultAsync(x => x.Id == id);
            if (user == null)
            {
                return NotFound();
            }
        ...
        [HttpPut]
        public async Task<ActionResult<Person>> Put(Worker user)
        {
            if (user == null)
            {
                return BadRequest();
        ...
        {
            Worker? user = db.Workers.FirstOrDefault(x => x.Id == id);
            if (user == null)
            {
                return NotFound();
            }
